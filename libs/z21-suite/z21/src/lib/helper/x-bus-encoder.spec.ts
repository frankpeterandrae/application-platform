...
describe('encodeXBusLanFrame', () => {
	...
	describe('frame without payload', () => {
		it('encodes no-payload frame as LAN_X followed by header', () => {
			const header = Z21LanHeader.LAN_GET_SERIAL_NUMBER;
		...
		it('encodes different headers correctly', () => {
			const header1 = Z21LanHeader.LAN_GET_SERIAL_NUMBER;
			const header2 = Z21LanHeader.LAN_GET_HWINFO;

			const buf1 = encodeXBusLanFrame(header1);
			const buf2 = encodeXBusLanFrame(header2);

			expect(buf1.readUInt16LE(2)).toBe(header1);
			expect(buf2.readUInt16LE(2)).toBe(header2);
			expect(buf1.readUInt16LE(2)).not.toBe(buf2.readUInt16LE(2));
		});
	});
