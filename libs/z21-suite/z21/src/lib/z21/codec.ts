 ...
 *   - Returns kind 'xbus', with `xHeader` extracted and `data` excluding the trailing XOR byte.
 *   - Logs a warning if computed XOR of [XHeader, DB...] doesn't match the final XOR byte, but still returns the dataset.
 * - 0x0084: System state. Payload must be exactly 16 bytes. Returns kind 'systemState'.
 *
 * Any other header (or malformed payload) yields kind 'unknown' to aid debugging and visibility of unexpected data.
 *
 ...
 */
export function parseZ21Dataset(buf: Buffer): Z21Dataset[] {
