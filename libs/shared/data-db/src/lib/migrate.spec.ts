...
describe('migrate', () => {
	...
	it('applies pending migrations in sorted order and records them', () => {
		const appliedRows = [{ id: 'paint:001_initial.sql' }];
		...
		expect(exec).toHaveBeenNthCalledWith(2, `-- ${join(migrationsDir, '002_seed_data.sql')}`);
		expect(exec).toHaveBeenNthCalledWith(3, `-- ${join(migrationsDir, '010_add_table.sql')}`);
		expect(insertRun).toHaveBeenCalledTimes(2);
		expect(insertRun).toHaveBeenNthCalledWith(1, 'paint:002_seed_data.sql', expect.any(String));
		expect(insertRun).toHaveBeenNthCalledWith(2, 'paint:010_add_table.sql', expect.any(String));
	});

	it('does not execute migration SQL when all files are already applied', () => {
		const appliedRows = [{ id: 'paint:001_initial.sql' }, { id: 'paint:002_seed_data.sql' }];
		const selectAll = vi.fn().mockReturnValue(appliedRows);
		const insertRun = vi.fn();
		const prepare = vi.fn((sql: string) => {
			if (sql.includes('SELECT id FROM migrations')) {
