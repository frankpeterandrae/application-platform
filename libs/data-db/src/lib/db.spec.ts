...
describe('openDb', () => {
	...
	it('uses explicit dbPath option over environment and default path', () => {
		...
		process.env['DB_PATH'] = 'D:/env/z21.db';

		openDb({ dbPath: 'D:/opts/custom.db' });

		expect(databaseCtorMock).toHaveBeenCalledWith('D:/opts/custom.db');
	});

	it('uses DB_PATH environment variable when option is not provided', () => {
		const dbMock = { pragma: vi.fn() } as unknown as Db;
		databaseCtorMock.mockReturnValue(dbMock);
		process.env['DB_PATH'] = 'D:/env/z21.db';
		...
		expect(databaseCtorMock).toHaveBeenCalledWith('D:/env/z21.db');
	});

	it('uses default data path when neither option nor environment variable are set', () => {
		const dbMock = { pragma: vi.fn() } as unknown as Db;
		databaseCtorMock.mockReturnValue(dbMock);

		openDb();

		expect(databaseCtorMock).toHaveBeenCalledWith(path.join(process.cwd(), 'data', 'z21.db'));
	});
	...
	it('creates parent directory before opening database file', () => {
		...
		openDb({ dbPath: 'D:/x/y/z21.db' });

		expect(fs.mkdirSync).toHaveBeenCalledWith(path.dirname('D:/x/y/z21.db'), { recursive: true });
	});
	...
	it('enables expected sqlite pragmas and returns database instance', () => {
		...
		const result = openDb({ dbPath: 'D:/x/z21.db' });
