...
describe('ClientMessageHandler.handle', () => {
	...
	beforeEach(() => {
		...
		z21Udp = { demoPing: vi.fn() } as any;
		handler = new ClientMessageHandler(locoManager as any, z21Udp as any, broadcast);
	});
	...
	it('ignores server.command.session.hello messages', () => {
		...
		expect(z21Udp.demoPing).not.toHaveBeenCalled();
	});

	it('handles system.command.trackpower.set by pinging Z21 and broadcasting power state', () => {
		handler.handle({ type: 'system.command.trackpower.set', on: true } as ClientToServer);
		expect(z21Udp.demoPing).toHaveBeenCalled();
	...
	it('updates turnout state and broadcasts on turnout.set', () => {
		handler.handle({ type: 'switching.command.turnout.set', addr: 9, state: 'thrown' } as unknown as ClientToServer);
		expect(z21Udp.demoPing).toHaveBeenCalled();
		expect(broadcast).toHaveBeenCalledWith({ type: 'switching.message.turnout.state', addr: 9, state: 'thrown' });
	});
